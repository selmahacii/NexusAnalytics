...
const INITIAL_REPORTS: Report[] = [
  {
    ...
    status: "final",
    content: {
      summary: "An exhaustive analysis of Q1 performance reveals a strong pivot towards the European wholesale market. While the UK remains the primary revenue driver, we noticed a significant 18% variance in profit margins for the 'Giftware' category due to rising supplier costs. The AI engine recommends a 5% price adjustment on low-margin high-volume items to offset logistics inflation.",
      kpis: [
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] animate-fade-in gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground font-medium">Detailed intelligence archives generated by Nexus AI and system audits.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="rounded-2xl h-11 border-white/10 bg-card/40 hover:bg-card/60 gap-2">
            <Calendar className="w-4 h-4" /> Date Filter
          </Button>
          <Button className="rounded-2xl h-11 gap-2 shadow-2xl shadow-primary/20 bg-primary hover:bg-primary/90">
      ...
      <div className="flex-1 flex gap-8 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto natural-scrollbar p-3 space-y-2">
            {filteredReports.map((report) => (
              <motion.button
                ...
                className={cn(
                  "w-full text-left p-5 rounded-[1.75rem] transition-all duration-300 group relative overflow-hidden",
                  selectedReportId === report.id
                    ? "bg-primary border border-primary/20 shadow-xl shadow-primary/10"
              ...
              >
                ...
                <h4 className={cn(
                  ...
                  selectedReportId === report.id ? "text-white" : "group-hover:text-foreground"
                )}>
                  {report.title}
                </h4>
                ...
                {selectedReportId === report.id && (
                  <motion.div layoutId="active-nav" className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-10 bg-white rounded-r-full" />
                )}
              </motion.button>
            ))}
          </div>
        ...
        <div className="flex-1 bg-card/20 backdrop-blur-3xl rounded-[3rem] border border-white/5 overflow-hidden flex flex-col shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] relative ring-1 ring-white/10">
          {/* Action Bar */}
          <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02] backdrop-blur-sm">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-2xl bg-primary shadow-lg shadow-primary/20 text-white">
                <FileSearch className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <h3 className="text-lg font-black tracking-tighter truncate leading-none">{selectedReport.title}</h3>
            ...
            <div className="max-w-5xl mx-auto space-y-12">
              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-12">
                <div className="space-y-6 flex-1">
                  <div className="flex items-center gap-3">
                    <Badge className="bg-primary/10 text-primary border-primary/20 text-[10px] font-black tracking-[0.2em] px-3 h-7 uppercase">Strategy Output</Badge>
                    <Badge variant="outline" className="text-[10px] font-black tracking-widest h-7 border-white/5 opacity-60 uppercase">{selectedReport.status}</Badge>
                  </div>
                  <h2 className="text-5xl font-black tracking-[calc(-0.05em)] text-premium-gradient">Nexus Intelligence Output</h2>
                  <p className="text-muted-foreground leading-relaxed text-base font-medium max-w-2xl">
                    {selectedReport.content.summary}
                  </p>
                </div>
                <div className="w-full lg:w-72 space-y-4">
                  <div className="p-6 rounded-[2.5rem] bg-gradient-to-b from-white/[0.05] to-transparent border border-white/5 flex flex-col gap-5 shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                       <Zap className="w-16 h-16" />
                    </div>
                    <div className="flex items-center gap-2">
                      <Brain className="w-4 h-4 text-primary" />
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Analytical Source</span>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm font-bold">Nexus AI Core (4o-Ensemble)</p>
                      <p className="text-[10px] font-medium text-emerald-500 uppercase tracking-widest">Decision Confidence: 98.4%</p>
                    </div>
              ...
              <Tabs defaultValue="highlights" className="w-full pt-8">
                ...
                <TabsContent value="anomalies" className="pt-10">
                  <div className="p-16 rounded-[4rem] border-2 border-dashed border-white/5 bg-white/[0.01] flex flex-col items-center justify-center text-center gap-8 shadow-inner">
                    <div className="w-20 h-20 rounded-[2.5rem] bg-amber-500/10 flex items-center justify-center text-amber-500 shadow-2xl relative">
                      <div className="absolute inset-0 bg-amber-500/20 blur-2xl rounded-full" />
                      <AlertTriangle className={cn("w-10 h-10 relative z-10", selectedReport.content.anomalies === 0 && "text-emerald-500")} />
                    </div>
                    <div className="max-w-md space-y-3">
                      <h4 className="text-2xl font-black tracking-tight">{selectedReport.content.anomalies} Logical Disruptions</h4>
                      <p className="text-sm text-muted-foreground font-medium leading-relaxed">
                        The Nexus Anomaly Detection Engine {selectedReport.content.anomalies > 0 ? "isolated patterns requiring structural attention" : "found no significant deviations"} in the source records for this period.
                      </p>
                    </div>
                    {selectedReport.content.anomalies > 0 && (
                      <Button variant="outline" className="h-12 px-8 rounded-2xl text-xs font-black uppercase tracking-widest gap-3 border-white/10 hover:bg-white/5">
                        <Eye className="w-4 h-4" /> Download Incident Log
                      </Button>
                    )}
                  </div>
                </TabsContent>
